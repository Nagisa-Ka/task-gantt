...
PyInstallerを使用してWindows向け実行ファイルを作成。 
GitHubの **Releases** からダウンロード可能。 
実行するとローカルサーバーが起動し、既定のブラウザでアプリケーションが自動的に開く。

### 終了方法

ブラウザを閉じてもバックグラウンドプロセスは終了しない。 
利用終了時は、タスクマネージャーから`GanttChartApp.exe`の終了が必要。